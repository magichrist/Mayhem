...
def _build_starter_drill(compose_path: Path | None) -> str:
    ...
    if compose_path is not None:
        ...
        if names:
            first = names[0]
            containers = {first: {"faults": [{"fault": "cpu.saturate", "percent": 50, "duration": "10s"}]}}  # noqa: E501
        else:
            containers = {"example": {"faults": [{"fault": "cpu.saturate", "percent": 50, "duration": "10s"}]}}  # noqa: E501
    else:
        containers = {"example": {"faults": [{"fault": "cpu.saturate", "percent": 50, "duration": "10s"}]}}  # noqa: E501
    doc: dict[str, object] = {
...
@click.option("--output", "output_path", type=click.Path(dir_okay=False, path_type=str), default=None, help="Output config path [default: mayhem.yaml].")  # noqa: E501
@click.option("--force", is_flag=True, help="Overwrite existing files.")
@click.pass_context
def init_cmd(ctx: click.Context, non_interactive: bool, output_path: str | None, force: bool) -> None:  # noqa: E501
    ...
    if not non_interactive and detection.explicit_engine is None:
        if detection.has_compose and detection.has_k8s_manifest:
            choice = click.prompt("select engine", type=click.Choice(["docker", "podman", "kubernetes"]), default=engine, show_default=True)  # noqa: E501
            engine = choice
        elif detection.has_k8s_manifest and not detection.has_compose:
            engine = click.prompt("select engine", type=click.Choice(["docker", "podman", "kubernetes"]), default="kubernetes", show_default=True)  # noqa: E501
        elif not detection.has_compose and not detection.has_k8s_manifest:
            engine = click.prompt("select engine", type=click.Choice(["docker", "podman", "kubernetes"]), default="docker", show_default=True)  # noqa: E501
    yaml_text = _build_starter_yaml(engine=engine, compose=compose_file, has_k8s=detection.has_k8s_manifest)  # noqa: E501
