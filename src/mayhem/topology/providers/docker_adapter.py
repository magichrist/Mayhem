"""DockerAdapter — concrete RuntimeAdapter for the Docker engine (ADR-M3-1).

Extracted from ``ContainerRuntimeProvider`` in docker_runtime.py.  Module-level
helpers are imported from that file; ``DockerAdapter`` wraps them behind the
normalised ``RuntimeAdapter`` and ``TopologyProvider`` interfaces.
"""
...
class DockerAdapter(RuntimeAdapter, TopologyProvider):
    """RuntimeAdapter for the local Docker engine."""
    ...
    @classmethod
    def best_effort(cls, engine: str | None = None) -> DockerAdapter | None:
        """Return a DockerAdapter if the engine is available, else None.

        Preserved for backward compat with callers that used
        ``ContainerRuntimeProvider.best_effort()``.
        """
        target = engine or cls.ENGINE
        adapter = cls(target)
    ...
    def capabilities(self) -> AdapterCapabilities:
        """Rootful Docker: all capabilities SUPPORTED."""
    ...
    def _filter_rows(
        ...
        """Apply compose / name filters to the raw ps rows."""
