...
def test_init_creates_mayhem_yaml_non_interactive(tmp_path, monkeypatch):
    runner = CliRunner()
...
def test_init_detects_compose(tmp_path, monkeypatch):
    runner = CliRunner()
    with monkeypatch.context() as mp:
        mp.chdir(tmp_path)
...
def test_init_detects_k8s_manifest(tmp_path, monkeypatch):
    runner = CliRunner()
    with monkeypatch.context() as mp:
        mp.chdir(tmp_path)
...
def test_init_generates_starter_drill_without_mutating_target(tmp_path, monkeypatch):
    runner = CliRunner()
    with monkeypatch.context() as mp:
        mp.chdir(tmp_path)
...
def test_init_never_calls_runtime(tmp_path, monkeypatch):
    runner = CliRunner()
    with monkeypatch.context() as mp:
        mp.chdir(tmp_path)
...
def test_init_existing_config_detected(tmp_path, monkeypatch):
    runner = CliRunner()
    with monkeypatch.context() as mp:
        mp.chdir(tmp_path)
        Path("mayhem.yaml").write_text("apiVersion: mayhem/v1\nruntime: podman\n")
