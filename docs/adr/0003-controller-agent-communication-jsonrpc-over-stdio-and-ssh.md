...
| **Controller-initiated sessions: newline-delimited JSON-RPC 2.0 over stdio; local = spawned child process, remote = persistent `ssh … tgondi-agent serve` exec channel** | Zero inbound ports anywhere; OS-authenticated transport (SSH keys); crash isolation between processes; frames readable in a terminal. Chosen. |
...
2. **Transports** behind one interface:
   - `LocalStdioTransport`: controller spawns `tgondi-agent serve --roles …` as a child process.
   - `SSHTransport`: persistent `ssh -o ControlMaster=auto -o ControlPersist=600 <host> --
     tgondi-agent serve`; stdout/stderr pipes carry protocol/log streams respectively.
...
5. **Remote bootstrap:** `tgondi agents install --host user@h` verifies Python ≥ 3.12, creates
   `/opt/tgondi/venv`, installs the wheel, optionally writes a systemd template unit
   (`tgondi-agent.service`) for root-mode agents; otherwise pinned sudo patterns apply
   ([ADR-0012](0012-safety-model-environment-identity-and-risk-gates.md)).

## Consequences

- **Positive:** trivially debuggable (`ssh host tgondi-agent serve | jq`); no firewall changes on
