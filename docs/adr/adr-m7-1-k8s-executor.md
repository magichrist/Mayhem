...
- The **safety gate flips** from blanket `k8s.unsupported` refusal to **per-family, capability-supervised admission**: a small admission gate evaluates the fault's `segment.family` — `proc`/`fd`/`mem`/`fs`/`cpu`(load.exec) — and admits only the exec-compatible subset of the catalog for pod targets)Skip. `k8s_node` targets remain refused by safety until k-plan-5 (node-level faults), matching k-plan-3 §scope boundary.
