...
`mayhem.yaml` — layered, versioned config ([ADR-0008](../adr/0008-layered-versioned-configuration.md)).
Precedence: CLI flags > env vars (`TGONDI_…`) > project `.mayhem/mayhem.yaml` > user
`~/.config/mayhem/mayhem.yaml` > defaults. Merges are deep; lists replace. Every snapshot pins its
resolved source map.
...
controller:
  state_dir: .mayhem             # SQLite + journals + artifacts
  log_level: info
...
agents:
  ssh:
    ...
    install_venv_path: /opt/mayhem/venv
...
- `apiVersion` must match a supported major; migrations upgrade files explicitly
  (`mayhem config migrate`) — never implicitly ([ADR-0008](../adr/0008-layered-versioned-configuration.md)).
...
```bash
mayhem config show          # resolved config with source annotations
mayhem config validate      # validate all layers without running anything
```
